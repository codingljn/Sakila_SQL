...
SUM(payment.amount) AS 'tot_rung_up'
FROM payment
...
SELECT film.title, COUNT(inventory.inventory_id) AS 'copies_in_inventory'
FROM film
...
JOIN
    ...
    JOIN (
		...
		WHERE country.country = 'Canada') AS city) ON city.city_id = address.city_id) AS address) ON address.address_id = customer.address_id;
...
JOIN
    ...
    JOIN (
		(SELECT category_id
		FROM category
		WHERE name = 'Family') AS category) ON category.category_id = film_category.category_id) AS film_category) ON film_category.film_id = film.film_id;
...
JOIN(
	(SELECT film_id
    FROM rental
    JOIN inventory ON inventory.inventory_id = rental.inventory_id) AS times_rented) ON times_rented.film_id = film.film_id
	GROUP BY film.title
	ORDER BY times_rented DESC;

-- 7f. Write a query to display how much business, in dollars, each store brought in.
SELECT store.store_id, SUM(payment.amount) AS 'tot_dollars'
FROM store
...
CREATE VIEW sakila.top_5_genres_revenue AS
SELECT name `Genres`, SUM(payment.amount) `Gross revenue`
FROM category
JOIN film_category ON film_category.category_id = category.category_id
JOIN inventory ON inventory.film_id = film_category.film_id
...
SELECT * FROM sakila.top_5_genres_revenue;
    
-- 8c. You find that you no longer need the view top_five_genres. Write a query to delete it.
DROP VIEW sakila.top_5_genres_revenue;